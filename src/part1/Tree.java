...
public class Tree {
	// Variables
	private String[] terms = { "X", "1" };
	private String[] functions = { "+", "-", "*", "/" };
	private int[] arityArr = { 2, 2, 2, 2 };
	private String[] primitives = { "+", "-", "*", "/", "X", "1" };
	private int[] primArr = { 2, 2, 2, 2, 0, 0 };
	...
	private void orderdepth(TreeNode<String> localRoot) {
		if (localRoot != null && localRoot.depth != 0) {
			System.out.print(localRoot.depth + "\t");
			int index = 0;

			while (index < localRoot.arity) {
				orderdepth(localRoot.getChild(index));
				index++;
			}
		}
	}

	private void orderID(TreeNode<String> localRoot) {
		if (localRoot != null && localRoot.depth != 0) {
			System.out.print(localRoot.id + "\t");
			int index = 0;

			while (index < localRoot.arity) {
				orderID(localRoot.getChild(index));
				index++;
			}
		}
	}
	...
	private void orderdepthString(TreeNode<String> localRoot) {
		if (localRoot != null && localRoot.depth != 0) {
			temp += (localRoot.depth + "\t");
			int index = 0;

			while (index < localRoot.arity) {
				orderdepthString(localRoot.getChild(index));
				index++;
			}
		}
	}

	private void orderIDString(TreeNode<String> localRoot) {
		if (localRoot != null && localRoot.depth != 0) {
			temp += (localRoot.id + "\t");
			int index = 0;

			while (index < localRoot.arity) {
				orderIDString(localRoot.getChild(index));
				index++;
			}
		}
	}
